...
returning id, number, capacity, status
`
...
func (q *Queries) CreateTable(ctx context.Context, arg CreateTableParams) (RestaurantTable, error) {
	row := q.db.QueryRowContext(ctx, createTable, arg.Number, arg.Capacity, arg.Status)
...
select id, number, capacity, status from restaurant_table
`
...
func (q *Queries) UpdateOrderItem(ctx context.Context, arg UpdateOrderItemParams) error {
	_, err := q.db.ExecContext(ctx, updateOrderItem, arg.ID, arg.Quantity)
	return err
}
